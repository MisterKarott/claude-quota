---
name: quota
description: This skill is triggered when the user runs "/quota" or asks to "check my quota", "show quota", "how much quota left", "Claude usage", "Claude Code usage". Displays a formatted view of current Claude Code rate limit usage.
allowed-tools:
  - Bash
  - Read
---
...
Display a clear, formatted breakdown of Claude Code rate limit usage for the current session.

## When to use

Trigger on `/quota` slash command or when the user explicitly asks about their current Claude Code quota, usage, or rate limits.
